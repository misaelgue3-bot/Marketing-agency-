...
/**
 * Lucía writes the client's month of social content: ready-to-paste captions
 * that the human approves in the Publicaciones calendar before publishing.
 */
  ...
  properties: {
    posts: {
      ...
      items: {
        ...
        required: ['caption'],
        properties: {
          caption: { type: 'string', description: 'The complete, ready-to-paste caption: hook line, 1-3 short sentences, a call to action, 2-4 fitting emojis woven in naturally, and 3-5 relevant hashtags on the last line.' },
        },
      },
    },
  },
...
async function generateContentMonth(client, { count = 8 } = {}) {
  const { json } = await llm.chat({
    system: LUCIA_SYSTEM,
    user:
      `Client:\n${clientBrief(client)}\n\n` +
      `Write ${count} social media captions (Instagram/Facebook) for this business's next month of content. ` +
      `Language: ${client.language === 'en' ? 'English' : 'natural Latin American Spanish (not translated English)'}. ` +
      'Mix the content types across the month: product/antojo posts, one promo/offer teaser (invent no prices — leave a ___ placeholder where the owner fills the offer), a behind-the-scenes/craft post, a customer-appreciation post, and a practical tip related to the business. ' +
      'Each caption stands alone, sounds like a proud local business owner (warm, never corporate), and ends with 3-5 relevant hashtags. Never invent specific prices, dates, addresses or claims.',
    maxTokens: 4000,
    schema: CONTENT_SCHEMA,
  });
  return (json.posts || []).slice(0, count).map((p) => String(p.caption || '').trim()).filter(Boolean);
}
