...
// Admin-controlled switches (Settings tab) override the .env defaults,
// so automations can be turned on/off without a redeploy.
const flag = (name) => {
  const s = store.db.settings.autoFlags;
  if (s && name in s) return Boolean(s[name]);
...
module.exports = { runTick, replyToLead, enabledFlags, isOn: flag };
