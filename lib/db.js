...
const EMPTY = { leads: [], clients: [], payments: [], campaigns: [], outbox: [], automations: {}, prospects: [], posts: [], settings: { ...DEFAULT_SETTINGS } };
...
module.exports = {
  ...
  // ---- content calendar: one row per social post to publish ----
  addPost(data) {
    const post = {
      id: id(),
      ...
      network: String(data.network || 'Instagram').slice(0, 40),
      caption: String(data.caption || '').trim().slice(0, 4000),
      status: ['por-hacer', 'listo', 'publicado'].includes(data.status) ? data.status : 'por-hacer',
      createdAt: new Date().toISOString(),
    };
    db.posts.push(post);
    persist(db);
  ...
  updatePost(postId, patch) {
    ...
    if (!p) return null;
    if ('date' in patch) p.date = String(patch.date || p.date).slice(0, 10);
    if ('network' in patch) p.network = String(patch.network || p.network).slice(0, 40);
    if ('caption' in patch) p.caption = String(patch.caption ?? p.caption).trim().slice(0, 4000);
    if ('status' in patch && ['por-hacer', 'listo', 'publicado'].includes(patch.status)) {
      p.status = patch.status;
      p.publishedAt = patch.status === 'publicado' ? new Date().toISOString() : undefined;
    }
    persist(db);
    return p;
  },
  ...
  updateSettings(patch) {
    // Automation switches: only known flags, coerced to booleans
    if (patch.autoFlags && typeof patch.autoFlags === 'object') {
      const KNOWN = ['AUTO_LEAD_REPLY', 'AUTO_MONTHLY_CHECKINS', 'AUTO_PAYMENT_REMINDERS', 'AUTO_CAMPAIGNS', 'WEEKLY_DIGEST'];
      db.settings.autoFlags = db.settings.autoFlags || {};
      for (const k of KNOWN) {
        if (k in patch.autoFlags) db.settings.autoFlags[k] = Boolean(patch.autoFlags[k]);
      }
    }
