...
/** Public URL of this deployment (Render sets RENDER_EXTERNAL_URL automatically). */
const appUrl = () => process.env.PUBLIC_URL || process.env.RENDER_EXTERNAL_URL || '';

/**
 * Validates Telegram Mini App initData (see core.telegram.org/bots/webapps):
 * data_check_string = sorted key=value pairs (minus hash) joined by \n;
 * secret = HMAC_SHA256(bot_token) keyed with "WebAppData";
 * valid when HMAC_SHA256(data_check_string, secret) === hash and auth_date is fresh.
 */
function verifyInitData(initData) {
  const token = process.env.TELEGRAM_BOT_TOKEN;
  if (!token || !initData || typeof initData !== 'string') return { ok: false };
  ...
  if (!authDate || Date.now() / 1000 - authDate > 24 * 60 * 60) return { ok: false };

  let user = null;
  try { user = JSON.parse(params.get('user') || 'null'); } catch { /* no user field */ }
  return { ok: true, user };
}
...
const sendMessage = (chatId, text, extra = {}) => api('sendMessage', { chat_id: chatId, text, ...extra });

/** Inline keyboard with the Sofía Mini App button (null when the public URL is unknown). */
const appKeyboard = () =>
  appUrl()
    ? {
        reply_markup: {
          inline_keyboard: [[
            { text: '✨ Abrir la app de Sofía / Open Sofía app', web_app: { url: `${appUrl()}/sofia-app.html` } },
          ]],
        },
      }
    : null;
...
const PRESS_BUTTON_HINT =
  '👇 Presiona el botón "✨ Abrir la app de Sofía" aquí abajo — el chat es más cómodo y puedes cambiar entre español e inglés. / ' +
  'Tap the "✨ Open Sofía app" button below — the chat is nicer there and you can switch between Spanish and English.';
...
async function handleMessage(msg, send = sendMessage) {
  ...
  if (text === '/start') {
    ...
    if (keyboard) {
      // Offer the Sofía Mini App and tell the customer to press the button
      await send(chatId, `${WELCOME}\n\n${PRESS_BUTTON_HINT}`, keyboard)
        .catch(() => send(chatId, WELCOME));
    } else {
      await send(chatId, WELCOME);
    }
    return;
  ...
  // First reply to a new customer also points them to the Sofía app button
  const keyboard = appKeyboard();
  if (isNew && keyboard) {
    await send(chatId, `${reply}\n\n${PRESS_BUTTON_HINT}`, keyboard).catch(() => send(chatId, reply));
  } else {
    await send(chatId, reply);
  }
...
module.exports = { start, stop, available, handleMessage, verifyInitData, notifyOwner, appUrl, CANNED_REPLY };
